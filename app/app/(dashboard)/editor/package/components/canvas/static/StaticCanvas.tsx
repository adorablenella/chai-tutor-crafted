...
const Canvas = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      const element = getElementByDataBlockId(document, first(ids));
      if (element) {
        const styleProp: string | null = element.getAttribute("data-style-prop");
        if (styleProp) {
          const styleId: string | null = element.getAttribute("data-style-id");
          const blockId: string | null = element.getAttribute("data-block-parent");
          setSelectedStylingBlocks([{ id: styleId, prop: styleProp, blockId }]);
        }
      }
    }, 100);
...
const StaticCanvas = (): React.JSX.Element => {
  ...
  const [selectedStyleElements, setSelectedStyleElements] = useState<HTMLElement[]>([]);
  ...
  useEffect(() => {
    if (!isEmpty(stylingBlocks) && iframeRef.current) {
      const selectedStyleElement = getElementByStyleId(iframeRef.current.contentDocument, first(stylingBlocks)?.id);
  ...
  return (
    <div
      ...
      <Frame
        ...
        <StylingHighlight selectedStyleElement={first(selectedStyleElements)} />
        <HeadTags model="page" />
        <Canvas>
          {loadingCanvas ? (

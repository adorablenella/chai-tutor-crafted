...
export default function PostCard({ data, projectData }: { data: any & { site: any | null }; projectData: any }) {
  const url = `${projectData?.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/${data?.slug}`;

  return (
    <div className="relative rounded-lg border border-stone-200 pb-10 shadow-md transition-all hover:shadow-xl dark:border-stone-700 dark:hover:border-white">
      <Link href={`/post/${data.id}`} className="flex flex-col overflow-hidden rounded-lg">
        ...
        <a
          href={
            process.env.NEXT_PUBLIC_VERCEL_ENV
              ? `https://${url}`
              : `http://${projectData?.subdomain}.localhost:3000/${data?.slug}`
          }
          target="_blank"
          rel="noreferrer"
